...
export class SaleslumenAppsScript implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Saleslumen Apps Script',
		name: 'saleslumenAppsScript',
		icon: { light: 'file:product.svg', dark: 'file:product.dark.svg' },
		group: ['transform'],
		version: [1],
		subtitle: '={{$parameter["operation"] + ": " + $parameter["resource"]}}',
		description: 'Create and run Saleslumen Apps Script projects',
		defaults: { name: 'Saleslumen Apps Script' },
		...
		properties: [
			...
			},
			{
				...
				options: [
					{ name: 'Create', value: 'create', action: 'Create project', description: 'Create a script project' },
					{ name: 'Get', value: 'get', action: 'Get project', description: 'Get a script project by ID' },
					{
						name: 'Update Content',
						value: 'updateContent',
			...
			},
			{
				...
				options: [
					{ name: 'Run', value: 'run', action: 'Run function', description: 'Run a script function' },
				],
				default: 'run',
			},
			{
			...
			},
			{
				...
				displayOptions: {
					show: {
						...
						operation: ['get', 'updateContent', 'run'],
					},
				},
			},
			{
				...
				displayOptions: { show: { resource: ['project'], operation: ['updateContent'] } },
			...
			},
			{
				...
				displayOptions: { show: { resource: ['function'], operation: ['run'] } },
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'project' && operation === 'create') {
				...
				} else if (resource === 'project' && operation === 'updateContent') {
					const scriptId = this.getNodeParameter('scriptId', i) as string;
					const content = parseJson(this, i, this.getNodeParameter('contentJson', i), 'Content') as IDataObject;
					response = (await saleslumenApiRequest.call(
						this,
						'script',
						{
				...
				} else if (resource === 'function' && operation === 'run') {
					const scriptId = this.getNodeParameter('scriptId', i) as string;
					const functionName = this.getNodeParameter('functionName', i) as string;
					const parameters = parseJson(this, i, this.getNodeParameter('parametersJson', i, '[]'), 'Parameters');
					response = (await saleslumenApiRequest.call(
						this,
						'script',
						{
					...
					if (response.error) {
						throw new NodeOperationError(
							...
							{
								description: 'Apps Script returned HTTP 200 with an Operation error. Check function logs and parameters.',
								itemIndex: i,
							},
						);
					}
